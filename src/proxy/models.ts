...
export function mapModelToClaudeModel(
  model: string,
  ...
  const sonnetOverride =
    process.env.MERIDIAN_SONNET_MODEL ?? process.env.CLAUDE_PROXY_SONNET_MODEL;
...
export async function getClaudeAuthStatusAsync(): Promise<ClaudeAuthStatus | null> {
  ...
  const ttl = cachedAuthStatusIsFailure
    ? AUTH_STATUS_FAILURE_TTL_MS
